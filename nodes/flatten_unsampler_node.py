...
class UnsamplerFlattenNode:
    ...
    def sample(self, model, add_noise, noise_seed, steps, old_qk, trajectories, cfg, sampler_name, scheduler, positive, negative, latent_image, end_at_step, return_with_leftover_noise, denoise=1.0):
        # PREPARTION
        latent = latent_image
        ...
        latent_image = sampler.sample(noise, positive, negative, cfg=cfg, latent_image=latent_image, force_full_denoise=False,
        ...
        latent_image = rearrange(latent_image, 'b c f h w -> (b f) c h w')

        return ({'samples': latent_image, 'injections': injection_dict},)
...
class Unsampler2:
    ...
    def unsampler(self, model, cfg, sampler_name, steps, end_at_step, scheduler, normalize, positive, negative, latent_image, trajectories):
        # PREPARTION
        latent = latent_image
        latent_image = latent["samples"]
        latent_image = rearrange(latent_image, "(b f) c h w -> b c f h w", b=1)
